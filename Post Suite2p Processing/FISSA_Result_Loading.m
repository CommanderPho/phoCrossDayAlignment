...
%         curr_cell_id_original_index = filteredCellFieldOriginalIndicies(i);
        ...
        curr_trial_fieldname = fissa_outputs.tifIdentifierNames{1}; % Just the first item
...
function [finalDataStruct] = fnPhoBuildUpdatedFDS_FromFISSA(fissa_outputs, finalDataStruct, phoPipelineOptions)
    ...
    assert((isfield(finalDataStruct, anmID)), 'anmID must be a valid animal ID in the finalDataStruct object!');
    ...
    assert((fissa_outputs.numCells == numComps), 'The number of cells must be the same in both the fissa_outputs and the extant finalDataStruct');
	...
	assert((leftover_frames == 0), 'leftover_frames > 0: Note that an incomplete session exists');
        % Session Indicies:
    ...
    assert((leftover_trials == 0), 'leftover_trials > 0: Note that an incomplete session exists');
	assert((numTrialsPerSession == num_trials_per_session), 'The number of trials per session computed from the FISSA data and that computed from the finalDataStruct differ!');
